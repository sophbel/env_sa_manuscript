...
# dev.off()
```
### base model random effects plots 
...
############# LAG EFFECT Plots ############
nointalltmp <- subset(nointall, nointall$cov%in%c("pm2p5","pm10","so2","hurs","absh","tasmin") & nointall$lag_num<9)
ggplot(nointalltmp)+
  geom_hline(yintercept=1, linetype="dashed",color="red")+
...
#### across lags
 ### across lags
   ...
   plot_agelag <- ggplot(fits_age_lag)+
      ...
      xlab(expression(paste('Concentration (', mu, 'g/m'^3, ')')))+
      ylab("Relative Risk")+
      scale_y_continuous(trans="log10", limits = c(0.8,1.5), breaks = c(0.8,1, 1.3))+
      # scale_y_continuous(trans="log10")+
...
dev.off()
   ...
   plot_dislag <- ggplot(fits_distypeslag)+
      ...
      theme(axis.text = element_text(size=13),axis.title=element_text(size=13), 
            strip.text = element_text(size=9), axis.text.x = element_text(angle = 45,hjust=1, size=13), 
            legend.position = "right")
   pdf(file="/home/sbelman/Documents/env_sa_manuscript/figures/figure4/disease_type_pm2p5_weekly.pdf", width = 12,height=3)
  print(plot_dislag)
  dev.off()
...
##### ART PCV PLOTS
```{r}
summary_df <- fread("/home/sbelman/Documents/env_sa_manuscript/figures/figure4/coverage_cat_summarydf.csv")
...
ggsave(file="/home/sbelman/Documents/env_sa_manuscript/figures/figure4/pcv_pm2p5_cumulative.png", width = 5,height=3)
    ...
    ## plot first 4 weeks
     fits_pcv_type_4 <- fits_pcv_type[which(fits_pcv_type$lag_num%in%0:8),]
...
p14
pdf(file="/home/sbelman/Documents/env_sa_manuscript/figures/figure5/cumulative_effect_pm2p5.pdf", width = 8,height=6)
print(p14)
dev.off()
```
...
p14 <- ggplot(main_gpsc)+
  ...
  theme(axis.text = element_text(size=10),axis.title=element_text(size=13), strip.text = element_text(size=13),
        strip.text.y = element_text(angle=0))
p14
pdf(file="/home/sbelman/Documents/env_sa_manuscript/figures/figure5/lagged_effect_pm2p5.pdf", width = 10,height=8)
print(p14)
dev.off()
...
pdf(file="/home/sbelman/Documents/env_sa_manuscript/figures/figure5/lagged_effect_selectGPSCs_pm2p5.pdf", width = 10,height=3)
print(p15)
dev.off()
table(data2[which(data2$GPSC=="26"),]$serotype) ## serotype 12F (NVT)
table(data2[which(data2$GPSC=="21"),]$serotype) ## serotype 19F (PCV7)
table(data2[which(data2$GPSC=="14"),]$serotype) ## serotype 23F (PCV7)
