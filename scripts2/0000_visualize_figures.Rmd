...
```{r}
# Relative humidity
tmphurs <- subset(fit_list, fit_list$cov%in%c("hurs") & fit_list$cov2%in%c("pm2p5","pm10","absh","tasmax","so2","none") & fit_list$lag_num %in% c(0:8) & fit_list$space_time=="weekly_adm2")
...
### pm2.5
tmppm2 <- subset(fit_list, fit_list$cov%in%c("pm2p5") & fit_list$cov2%in%c("absh","hurs","tasmax","so2","none") & fit_list$lag_num %in% c(0:12) & fit_list$space_time=="weekly_adm2")
tmppm2$cov2 <- factor(tmppm2$cov2, levels = c("absh","hurs","tasmax","so2","none"))
...
### PM10
tmppm1 <- subset(fit_list, fit_list$cov%in%c("pm10") & fit_list$cov2%in%c("absh","hurs","tasmax","so2","none") & fit_list$lag_num %in% c(0:12) & fit_list$space_time=="weekly_adm2")
tmppm1$cov2 <- factor(tmppm1$cov2, levels = c("absh","hurs","tasmax","so2","none"))
...
# ABSH
tmpabsh <- subset(fit_list, fit_list$cov%in%c("absh") & fit_list$cov2%in%c("hurs","pm2p5","pm10","tasmax","so2","none") & fit_list$lag_num %in% c(0:12) & fit_list$space_time=="weekly_adm2")
...
# SO2
tmpso2 <- subset(fit_list, fit_list$cov%in%c("so2") & fit_list$cov2%in%c("hurs","absh","pm2p5","pm10","tasmax","none") & fit_list$lag_num %in% c(0:12) & fit_list$space_time=="weekly_adm2")
tmpso2$cov2 <- factor(tmpso2$cov2, levels = c("pm2p5","pm10","absh","hurs","tasmax","none"))
...
# PM2.5
tmp <- subset(fit_list, fit_list$cov%in%c("pm2p5") & fit_list$cov2%in%c("absh","hurs","tasmax","so2","none") & fit_list$lag_num %in% c(3) & fit_list$space_time==space_time_str)
tmp$cov2 <- factor(tmp$cov2, levels = c("absh","hurs","tasmax","so2","none"))
plot_pm2p5_cum <- ggplot(tmp)+
  ...
  theme(axis.text = element_text(size=13),axis.title=element_text(size=13), 
        ...
        legend.position = "right")
# PM10
tmp <- subset(fit_list, fit_list$cov%in%c("pm10") & fit_list$cov2%in%c("absh","hurs","tasmax","so2","none") & fit_list$lag_num %in% c(3) & fit_list$space_time==space_time_str)
tmp$cov2 <- factor(tmp$cov2, levels = c("absh","hurs","tasmax","so2","none"))
plot_pm10_cum <- ggplot(tmp)+
  ...
  theme(axis.text = element_text(size=13),axis.title=element_text(size=13), 
        ...
        legend.position = "right")
# HURS
tmp <- subset(fit_list, fit_list$cov%in%c("hurs") & fit_list$cov2%in%c("pm2p5","pm10","tasmax","so2","none") & fit_list$lag_num %in% c(3) & fit_list$space_time==space_time_str)
...
plot_hurs_cum <- ggplot(tmp)+
  ...
  theme(axis.text = element_text(size=13),axis.title=element_text(size=13), 
        ...
        legend.position = "right")
# ABSH
tmp <- subset(fit_list, fit_list$cov%in%c("absh") & fit_list$cov2%in%c("pm2p5","pm10","tasmax","so2","none") & fit_list$lag_num %in% c(3) & fit_list$space_time%in%space_time_str)
tmp$cov2 <- factor(tmp$cov2, levels = c("pm2p5","pm10","so2","tasmax","none"))
...
# Sulfur Dioxide
tmp <- subset(fit_list, fit_list$cov%in%c("so2") & fit_list$cov2%in%c("pm2p5","pm10","tasmax","absh","none") & fit_list$lag_num %in% c(3) & fit_list$space_time%in%space_time_str)
tmp$cov2 <- factor(tmp$cov2, levels = c("pm2p5","pm10","so2","tasmax","absh","none"))
...
# mod_gof1<-fread("/home/sbelman/Documents/env_sa_manuscript/models/dlnms/suboutcomes/serotypes/mod_gof_dlnm_alloutcomes_weekly_adm2_serotype_2019.csv")
# mod_gof2<-fread("/home/sbelman/Documents/env_sa_manuscript/models/dlnms/suboutcomes/demographic/mod_gof_dlnm_alloutcomes_weekly_adm2_demographic_2019.csv")
...
df <- df %>%
  ...
  mutate(week_seq_prov = sum(sequenced), .groups = 'drop') 
...
# Compute correlation per province on RAW CONCENTRATIONS
            cor_by_province <- df %>%
              group_by(NAME_1) %>%
            ...
            # # Plot
            # corr_conc <- ggplot(cor_by_province, aes(x = Lag, y = Correlation, color = Type)) +
...
### COMPUTE CORRELATION ON SPIKES
# function to flag spikes based on peak detection
detect_spikes <- function(x, threshold = 40) {
  x <- as.numeric(x)
  ...
  spike_flag <- rep(0, length(x))
  if (!is.null(peaks)) {
    idx <- peaks[,2]  # center index of peaks
    spike_flag[idx] <- 1
  }
...
### assign spikes
for (col in lag_cols) {
  df[[paste0(col, "_spike")]] <- detect_spikes(df[[col]])
}


### correlation by spikes
spike_cols <- grep("_spike$", names(df), value = TRUE)
cor_by_province_spikes <- df %>%
  group_by(NAME_1) %>%
  summarise(
    across(all_of(spike_cols), ~ cor(nvt_prop, ., use = "complete.obs"), .names = "NVT_{.col}"),
    across(all_of(spike_cols), ~ cor(pcv7_prop, ., use = "complete.obs"), .names = "PCV7_{.col}"),
    across(all_of(spike_cols), ~ cor(pcv13_prop, ., use = "complete.obs"), .names = "PCV13_{.col}"),
    across(all_of(spike_cols), ~ cor(GPSC21_count/week_seq_prov, ., use = "complete.obs"), .names = "GPSC21_{.col}")
  ) %>%
  pivot_longer(
    cols = -NAME_1,
    names_to = c("Type", "Lag"),
  ...
  mutate(Lag = as.numeric(gsub("_spike", "", Lag)))

corr_spike <- ggplot(cor_by_province_spikes, aes(x = Lag, y = Correlation, color = Type)) +
                geom_hline(yintercept = 0, linetype = "dashed", color = "black")+
              geom_line(size = 1.2) +
              geom_point(size = 2) +
              facet_wrap(~ NAME_1) +
...
#### deconvolute pm and find peaks
```{r}
library(data.table)
library(dplyr)
library(pracma)

df <- fread("/home/sbelman/Documents/env_sa_manuscript/dataframes/sa_adm1_weekly_lag.csv")
df <- data.frame(df)
