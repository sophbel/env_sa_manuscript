...
#### FIGURE 2 ATTRIBUTABLE FRACTION
```{r}
...
#### FIGURE S3 AND S4
#### Gauteng Alone Compared to South Africa Sensitivity
...
noint$cov2 <- "none"
noint <- subset(noint, noint$covariate%notin%c("pm2p5_lag0","pm10_lag0","so2_lag0"))
## pollution only
...
# Relative humidity
tmphurs <- subset(fit_list, fit_list$cov%in%c("hurs") & fit_list$lag_num %in% c(3))
...
### pm2.5
tmppm2 <- subset(fit_list, fit_list$cov%in%c("pm2p5") & fit_list$lag_num %in% c(3))
...
### PM10
tmppm1 <- subset(fit_list, fit_list$cov%in%c("pm10")  & fit_list$lag_num %in% c(3))
...
# ABSH
tmpabsh <- subset(fit_list, fit_list$cov%in%c("absh")  & fit_list$lag_num %in% c(3))
...
### save each individually
pdf(paste0("/home/sbelman/Documents/env_sa_manuscript/figures/figure3/prlr_cumulative_additive_sa_",space_time_str,".pdf"),width  =4.5,height=3.5)
print(plot_prlr_cum)
dev.off()
