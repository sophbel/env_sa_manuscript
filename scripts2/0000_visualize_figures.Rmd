...
p14
pdf(file=paste0("/home/sbelman/Documents/env_sa_manuscript/figures/figure5/cumulative_effect_pm2p5",space,".pdf"), width = 8,height=6)
print(p14)
dev.off()
```
...
main_gpsc$lagweek <- factor(main_gpsc$lagweek, levels = c("week 0", "week 1","week 2","week 3","week 4","week 5","week 6","week 7","week 8"))
p14 <- ggplot(main_gpsc)+
  ...
  theme(axis.text = element_text(size=10),axis.title=element_text(size=13), strip.text = element_text(size=13),
        strip.text.y = element_text(angle=0))
p14
pdf(file= paste0("/home/sbelman/Documents/env_sa_manuscript/figures/figure5/lagged_effect_pm2p5_",space,".pdf"), width = 10,height=8)
print(p14)
dev.off()


#### select GPSCs
p15 <- ggplot(main_gpsc[which(main_gpsc$GPSC%in%c("GPSC14","GPSC21","GPSC26")),])+
  geom_line(aes(x=var,y=exp(fit), group=interaction(cov, GPSC, interaction_level,lagweek),  color=interaction_level))+
  geom_hline(yintercept = 1, linetype = "dashed", color="red", alpha=0.6)+
  geom_ribbon(aes(x=var, ymin=exp(lowerCI),ymax=exp(upperCI), group=interaction(cov, GPSC, interaction_level,lagweek), fill=interaction_level),alpha=0.2)+
  scale_color_manual(values = c("high"="red","low"="blue", "med" = "darkgreen")) +
...
#### serotype 19F types
p16 <- ggplot(main_gpsc[which(main_gpsc$GPSC%in%c("GPSC1","GPSC21")),])+
  geom_line(aes(x=var,y=exp(fit), group=interaction(cov, GPSC, interaction_level,lagweek),  color=interaction_level))+
  geom_hline(yintercept = 1, linetype = "dashed", color="red", alpha=0.6)+
  geom_ribbon(aes(x=var, ymin=exp(lowerCI),ymax=exp(upperCI), group=interaction(cov, GPSC, interaction_level,lagweek), fill=interaction_level),alpha=0.2)+
  scale_color_manual(values = c("high"="red","low"="blue", "med" = "darkgreen")) +
