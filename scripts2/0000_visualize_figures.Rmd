...
#### calculate N
```{r}
sa_adm2_weekly<-fread(file="/home/sbelman/Documents/BRD/SouthAfrica/data/sa_adm2_weekly.csv")
sa_adm1_weekly<-fread(file="/home/sbelman/Documents/BRD/SouthAfrica/data/sa_adm1_weekly.csv")
sa_adm1_weekly %>%
  group_by(province) %>%
...
shp<-st_read("/home/sbelman/Documents/BRD/SouthAfrica/shps/gadm41_namematch_ZAF_2.shp")
basemod <- readRDS(file=paste0("/home/sbelman/Documents/BRD/SouthAfrica/models/outputs/base_models/base_model_",time,"_20092011_popdens_",space,"_2023.rds"))
## spatial effects
N=52
mod<-basemod
  shp_a1<-shp
...
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/random_effects/res_weekly_adm2.pdf", width = 12,height=4)
print(spat+m+y)
dev.off()
```
...
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure1/dis_inc.pdf", width = 7,height=5)
print(displot)
dev.off()
...
```{r}
sa_adm2_weekly<-fread(file="/home/sbelman/Documents/BRD/SouthAfrica/data/sa_adm2_monthly.csv")
...
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure1/env_ts.pdf", width = 9,height=7)
# print(envs)
# dev.off()
...
age_cols <- c("age_lt5", "age_5t14", "age_14t64", "age_gt65")
...
soc_data <- sa_adm2_weekly %>%
  group_by(date) %>%
  summarise(across(
    all_of(c(disease_cols, age_cols, type_cols, vaccine_cols, sero_cols)),
    ~ sum(.x, na.rm = TRUE)
  ), .groups = "drop") %>%
  pivot_longer(
    cols = -c(date),
  ...
  mutate(
    facet_group = case_when(
      ...
      variable %in% sero_cols ~ "Serotype",
      TRUE ~ "Other"
    )
  )
...
write.csv(yearly_summary, "/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure1/yearly_summary.csv", row.names = FALSE)

## factor for order
soc_data$facet_group <- factor(soc_data$facet_group, 
                               levels = c("Disease","Age Group", "Disease Type", "Vaccine Type","Serotype"),
                               labels = c("Disease","Age\nGroup", "Disease\nType", "Vaccine\nType","Serotype"))
soc_data$variable <- factor(soc_data$variable, 
                               levels = c(disease_cols, age_cols, type_cols, vaccine_cols, sero_cols))
## remove count label
...
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure1/soc_ts.pdf", width = 6,height=7)
print(socts)
dev.off()
```
#### disease separately with particulate matter and hurs
```{r}
sa_adm1_monthly<-fread(file="/home/sbelman/Documents/BRD/SouthAfrica/data/sa_adm1_monthly.csv")
...
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure1/disenv_ts.pdf", width = 9,height=4)
# print(dis_env)
# dev.off()
...
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure1/dispmhurs_ts.pdf", width = 8,height=5)
...
```{r}
data<-fread("/home/sbelman/Documents/BRD/SouthAfrica/disease/SA_disease_point.csv")
weekly_data <- data %>%
...
# ggsave("./data_summary/country_daily.png",width=10,height=8)
####### Montly Plot
...
pdf("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/weeklymonthlyprov_casecount.pdf", width=12,height=10)
(weekly_count + month_count) /(prov_count+intplot) 
dev.off()
# (weekly_count + month_count) /(prov_count+intplot) + 
#   patchwork::plot_annotation(tag_levels = 'a')&
#   theme(plot.tag = element_text(size = 25))
# ggsave("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/weeklymonthlyprov_casecount.png", width=12,height=4)
...
pprov<-ggplot(shp)+
  ...
  labs(color="Province")
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure1/hosp_pprov.pdf", width = 7,height=5)
# print(pprov)
# dev.off()
```
...
```{r}
### adm1
data_unscaled <- fread("/home/sbelman/Documents/BRD/SouthAfrica/data/sa_adm2_weekly_lag.csv")
...
data_unscaled <- fread("/home/sbelman/Documents/BRD/SouthAfrica/data/sa_adm1_weekly_lag.csv")
color_labels = c("Eastern Cape" = "#008080", "Free State" = "#D75F00", "Gauteng" = "#967bb6" , "KwaZulu-Natal"= "#FFCCCB",
                     "Limpopo" = "#228B22", "Mpumalanga" = "#FFC000", "North West" = "#8B5A2B", "Northern Cape" = "#606060", "Western Cape" = "#1A2D6D" )
...
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figures/envtmpvhum_descriptions.pdf", width = 4,height=4)
tmpvhum
dev.off()
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figures/env_descriptions.pdf", width = 13,height=4)
(weeksover +  + hist_pm25) + 
  patchwork::plot_annotation(tag_levels = 'a')
dev.off()
(weeksover + hist_pm25) + 
  patchwork::plot_annotation(tag_levels = 'a')
ggsave(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figures/env_descriptions.png", width = 10,height=4)
```
...
```
### Plot with lags across 12 weeks
```{r}
# Relative humidity
tmphurs <- subset(fit_list, fit_list$cov%in%c("hurs") & fit_list$cov2%in%c("pm2p5","pm10","absh","tasmax","tasmin","none") & fit_list$lag_num %in% c(0:12) & fit_list$space_time=="weekly_adm2")
...
pdf(paste0("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/cumulative_additive_sa_",space_time_str,".pdf"),width  =9,height=7)
...
(plot_pm2p5_cum + plot_pm10_cum)/
  (plot_hurs_cum + plot_absh_cum)
  ggsave(file = paste0("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/cumulative_additive_sa_",space_time_str,".png"), width = 9, height = 7)
  }else{
   (plot_pm2p5_cum + plot_pm10_cum + plot_hurs_cum )
    ggsave(file = paste0("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/cumulative_additive_sa_",space_time_str,".png"), width = 13, height = 4)

  }

# Maximum temperature
space_time_str <- "weekly_adm1"
...
plot_temp_cum
# ggsave("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/cumulative_temperature_maxmin_weekly_adm2.png", width = 7, height = 5)
```
...
noint$cov2 <- "none"
biv_noint <- fread(file=paste0("/home/sbelman/Documents/BRD/SouthAfrica/models/outputs/dlnms/nointeraction_results_fits_",time,"_",space,"_bivariable_12week_mixeddf_",prov_name,".csv"))
...
(plot_hurs+plot_absh)
ggsave(paste0("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/cumulative_additive_gauteng_weekly_",space,".png"), width = 9, height = 7)
...
ggsave("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/gauteng_vs_southafrica_lags.png",width=15,height=15)
```
...
```{r}
  ...
  plot_age <- ggplot(fits_age_cum)+
      ...
      theme(axis.text = element_text(size=13),axis.title=element_text(size=13), 
            ...
            legend.position = "right")
    plot_age
    ggsave(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/age_pm2p5_cumulative.png", width = 5,height=3)
    
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/age_pm2p5_cumulative.pdf", width = 5,height=3)
# print(plot_age)
# dev.off()
...
```{r}
  ...
  plot_dis <- ggplot(fits_distypes_cum)+
      ...
      theme(axis.text = element_text(size=13),axis.title=element_text(size=13), 
            ...
            legend.position = "right")
    plot_dis
    ggsave(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/disease_type_pm2p5_cumulative.png", width = 5,height=3)
    
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/disease_type_pm2p5_cumulative.pdf", width = 5,height=3)
# print(plot_dis)
# dev.off()
...
##### ART PCV PLOTS
```{r}
summary_df <- fread("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure2/coverage_cat_summarydf.csv")
...
year_eff_base <- fread("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure2/year_eff_base.csv")
...
pdf("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure2/random_national_yearlyRR.pdf", width = 6, height =3)
print(random_national)
dev.off()

pdf("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/figure2/RR_sumcat.pdf", width = 4.5, height =4)
print(sum_cat)
dev.off()
```
...
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/serotype_pm2p5_cumulative.pdf", width = 10,height=3)
# print(plot_sero)
# dev.off()
plot_sero
ggsave(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/serotype_pm2p5_cumulative.png", width = 12,height=5)
...
metrics_out <- ggplot(mod_gof_long)+
  ...
  ylab("Value")
metrics_out
ggsave("/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/model_performances.png")
```
...
### plot cumulative
  ...
  plot_pcv_cum <- ggplot(fits_pcv_type_cum)+
      ...
      theme(axis.text = element_text(size=13),axis.title=element_text(size=13), 
            ...
            legend.position = "right")
    plot_pcv_cum
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/pcv_pm2p5_cumulative.pdf", width = 5,height=3)
# print(plot_pcv_cum)
# dev.off()
plot_pcv_cum
ggsave(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/pcv_pm2p5_cumulative.png", width = 5,height=3)
  ...
  plot_pcv_4 <- ggplot(fits_pcv_type_4)+
      ...
      theme(axis.text = element_text(size=13),axis.title=element_text(size=13), 
            ...
            legend.position = "right")
  plot_pcv_4
#   pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/pcv_pm2p5_6weeks.pdf", width = 12,height=3)
# print(plot_pcv_4)
# dev.off()
plot_pcv_cum
ggsave(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/pcv_pm2p5_cumulative.png", width = 10,height=3)
...
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/suboutcome_models/rrratio_suboutcomes.pdf", width = 5,height=3)
print(prr)
dev.off()
```
...
hursp <- ggplot(hursplot2)+
  ...
  facet_grid(.~var)
(pm2p5+pm10) 
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/cumulative_rrRatio_pm2p5.pdf", width = 4,height=3)
# print(pm2p5) 
# dev.off()
```
...
hursp <- ggplot(hursplot2)+
  ...
  facet_grid(.~var)
(pm2p5+pm10) 
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/lag3_rrRatio_pm2p5.pdf", width = 4,height=3)
# print(pm2p5) 
# dev.off()
```
...
p14
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/cumulative_effect_pm2p5.pdf", width = 8,height=6)
print(p14)
dev.off()
```
...
p14 <- ggplot(main_gpsc)+
  ...
  theme(axis.text = element_text(size=10),axis.title=element_text(size=13), strip.text = element_text(size=13),
        strip.text.y = element_text(angle=0))
p14
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/lagged_effect_pm2p5.pdf", width = 10,height=8)
print(p14)
dev.off()
```
...
p12 <- ggplot(allfits_pivoted)+
  ...
  theme(axis.text = element_text(size=13),axis.title=element_text(size=13), strip.text = element_text(size=13),
        strip.text.y = element_text(angle=0))
p12
# ggsave(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/cumulative_effect__select_pm2p5.png", width = 6,height=6)
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/cumulative_effect__select_pm2p5.pdf", width = 6,height=6)
# print(p12)
# dev.off()
...
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/cumulative_effect_prepostsensitivity_select_pm2p5.pdf", width = 6,height=6)
# print(p14)
# dev.off()
# p14
# ggsave(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/cumulative_effect_prepostsensitivity_select_pm2p5.png", width = 8,height=6)
...
# pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/proportionserotype_selectGPSCs_pm2p5.pdf", width = 4,height=6)
# print(p19f/p12f/p6A)
# dev.off()
```
...
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/gpsc21gpsc1gpsc3_6week_pm2p5.pdf", width = 9,height=6)
...
pdf(file="/home/sbelman/Documents/BRD/SouthAfrica/manuscript/interaction/waic_interactions_pm2p5.pdf", width = 4,height=3)
print(waic) 
dev.off()
...
```{r}
df <- fread(file="/home/sbelman/Documents/BRD/SouthAfrica/data/sa_adm1_weekly_lag.csv")
df <- data.frame(df)
