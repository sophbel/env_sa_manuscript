...
base_pcvart_models <- c(base_pcvart_model_list, base_pcvartprovrep_model_list, base_pcvart_nvtmodel_list)

mod_out <- lapply(base_pcvart_models, function(mod) eval.mod(mod, df)) # output model evaluation metrics as a list
...
mod_out$rsq <- apply(mod_out, 1, function(x) rsq(mod_out[mod_out$cov == x["cov"], ], base_pcvart_models[[1]], num_outcomes = 1))
## clean table
...
write.table(mod_out,file="/home/sbelman/Documents/env_sa_manuscript/models/pcvart_models/base_pcvart_summary_statisics_weekly.csv",quote=FALSE,row.names=FALSE,col.names=TRUE)

# ################################################################################
# #### LOAD MODELS AND ASSIGN THEM TO NAMES
#### PLOTS FOR THE EXPERIMENT
# ################################################################################
## load the GOFs from the models
mod_out <- fread(file="/home/sbelman/Documents/env_sa_manuscript/models/pcvart_models/base_pcvart_summary_statisics_weekly.csv")
...
fixedeffs$model <- factor(fixedeffs$model, levels = c("ART_Vax","ART","PCV","vaccine_periods","2009vaccine"))
...
# ################ FIXED EFFECTS LINEAR - NO PROVINCE REPLICATION -- NON VACCINE TYPES ##############
fixed_effects <- rbind(modART_nvt$summary.fixed[2,],
...
fixedeffs <- cbind(fixed_effects, model = c("ART","PCV", "vaccine_periods", "2009vaccine","ARTnational","ART_Vax","ART_Vax"), effect = c("ART","PCV","vaccine_periods","2009vaccine","ARTnational","vaccine_periods","ART"))
fixedeffs$model <- factor(fixedeffs$model, levels = c("ART_Vax","ART","PCV","vaccine_periods","2009vaccine", "ARTnational"))

colnames(fixedeffs) <- c("mean","sd","lowerCI","median","upperCI","mode","kld","model","effect")
fixedeffs_rr <-fixedeffs
# fixedeffs[, 1:5] <- exp(fixedeffs[,1:5])
color_labels <- c("ART" = "#097969","PCV" = "#D91656", "ART_Vax" = "darkblue","ARTnational"="grey", "vaccine_periods"="orange","2009vaccine"="purple" )

fixed_national_nvts <- ggplot(fixedeffs)+
  geom_point(aes(x=effect,y=median, group=model, color= model ),size=3, position=position_dodge(width=0.5)) +
  geom_errorbar(aes(x=effect,ymin=lowerCI,ymax=upperCI, group=model, color= model), width=.3, position=position_dodge(width=0.5))+
  theme_bw()+
  xlab("Intervention")+
...
# ################ FIXED EFFECTS LINEAR - NO PROVINCE REPLICATION --VACCINE TYPES ##############
fixed_effects <- rbind(modART_vts$summary.fixed[2,],
...
fixed_national_vts <- ggplot(fixedeffs)+
  geom_point(aes(x=effect,y=median, group=model, color= model ),size=3, position=position_dodge(width=0.5)) +
  geom_errorbar(aes(x=effect,ymin=lowerCI,ymax=upperCI, group=model, color= model), width=.3, position=position_dodge(width=0.5))+
  theme_bw()+
  xlab("Intervention")+
  ggtitle("Vaccine Types alone")+ 
  ylab("Effect")+
  ylim(-4,0.1)+
  geom_hline(yintercept=0,linetype="dashed",color="red")+
  theme(axis.text.x = element_text(angle=45,hjust=1, size=13), axis.text = element_text(size=13), axis.title = element_text(size=13)) +
...
fixed_national_nvts + fixed_national_vts + fixed_national_all
